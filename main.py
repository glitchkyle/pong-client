...
def playGame(client: socket, game_state: GameState) -> None:
    ...
    while True:
        ...
        def drawCenteredMessage(rect:pygame.Rect,font:pygame.font.Font,message:str,antialias:bool,center:tuple[float, float]) -> pygame.Rect:
            textSurface = font.render(message,antialias,Color.WHITE.value, (0,0,0))
        ...
        if left_score > MAX_SCORE - 1 or right_score > MAX_SCORE - 1:
            winText = "Player 1 Wins! " if left_score > MAX_SCORE - 1 else "Player 2 Wins! "
            drawCenteredMessage(win_message,win_font,winText,False,((screen_width/2), screen_height/2))
            if current_game_state.again[current_game_state.player_id] == 0:
                playAgainMessage = drawCenteredMessage(playAgainMessage,win_font,"Play Again",True,((screen_width/2), screen_height/2+90))
            
            if current_game_state.again[int(not current_game_state.player_id)] == 1:
                playAgainMessage = drawCenteredMessage(playAgainMessage, win_font,f"Your opponent wants a rematch",True,((screen_width/2), screen_height/2+90))
        else:
